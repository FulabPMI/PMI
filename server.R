server <- function(input, output, session) {
  ...
  model_results <- reactiveValues(
    ...
    current_actual_predicted_plot = NULL,
    current_residual_plot = NULL,
    current_feature_importance_plot = NULL,
    current_transfer_learning_plot = NULL
  )
  ...
  observe({
    ...
    if (!is.null(tissue_input) && tissue_input %in% unique(data$Tissue)) {
      circRNA_choices <- sort(unique(data$circRNA[data$Tissue == tissue_input]))
  ...
  predict_pmi <- function(tab) {
    ...
    withProgress(message = "Making prediction", value = 0, {
      ...
      if (tab %in% c("mouse", "human")) {
        ...
        model_key <- paste(tissue, normalize_circRNA(circRNA), model_results$current_model_type, transfer_flag, sep = "_")
        ...
        filtered_data <- filter_by_case_insensitive_circRNA(data, tissue, circRNA)
        ...
        filtered_data <- filter_by_case_insensitive_circRNA(data, input$tissue_user, input$circRNA_user)
  ...
  train_transfer_model_specific <- function(target_tissue, target_circRNA) {
    ...
    tryCatch({
      ...
      if (current_tab == "mouse") {
        ...
        mouse_filtered <- filter_by_case_insensitive_circRNA(mouse, target_tissue, target_circRNA)
      ...
      if (current_tab == "human") {
        ...
        human_filtered <- filter_by_case_insensitive_circRNA(human, target_tissue, target_circRNA)
  ...
  create_actual_predicted_plot <- function() {
    ...
    p <- ggplot(df, aes(x = PMI_Hour, y = predicted)) +
    ...
    model_results$current_actual_predicted_plot <- p
    return(p)
  }
  ...
  create_residual_plot <- function() {
    ...
    model_results$current_residual_plot <- p
    return(p)
  }
  ...
  create_feature_importance_plot <- function() {
    ...
    tryCatch({
      ...
      model_results$current_feature_importance_plot <- p
      return(p)
  ...
  create_3d_plot <- function() {
    ...
    filtered_data <- filter_by_case_insensitive_circRNA(data, tissue_id, circ_id)
    ...
    training_sample_count <- nrow(filtered_data)
    ...
    predicted_pmi <- ifelse(model_results$predicted_value < 0, 0, model_results$predicted_value)
    ...
    p <- p %>%
      add_trace(
        ...
        type = "scatter3d",
        mode = "markers",
        marker = list(color = "red", size = 12, symbol = "diamond", opacity = 1),
        text = paste(
          "Tissue:", tissue_id, "<br>",
          "circRNA:", circ_id, "<br>",
          "Expression:", round(new_sample_expr, 3), "<br>",
          "Temperature:", round(new_sample_temp, 1), "°C<br>",
          "<b>Predicted PMI:", round(predicted_pmi, 2), "hours</b>"
        ),
        hoverinfo = "text",
        name = "Predicted Sample"
      ) %>%
      layout(
        title = paste("3D Plot for", tissue_id, "-", circ_id, "<br>Training Samples:", training_sample_count),
        scene = list(
          xaxis = list(title = "Expression", titlefont = list(size = 12)),
          yaxis = list(title = "PMI (hours)", titlefont = list(size = 12)),
          zaxis = list(title = "Temperature (°C)", titlefont = list(size = 12)),
  ...
  output$transfer_learning_plot <- renderPlot({
    ...
    tryCatch({
      ...
      human_prep <- filter_by_case_insensitive_circRNA(human, current_tissue, current_circRNA) %>%
      ...
      model_key <- paste(current_tissue, normalize_circRNA(current_circRNA), selected_model_type, "human_only", sep = "_")
      ...
      if (transfer_enabled) {
        ...
        mouse_prep <- filter_by_case_insensitive_circRNA(mouse, current_tissue, current_circRNA) %>%
          dplyr::select(PMI_Hour, Expression, Temperature)
      ...
      model_results$current_transfer_learning_plot <- p
      return(p)
